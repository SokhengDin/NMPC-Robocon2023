...
def game_field_plot():
    ...
    gf_width = 12.0
    gf_height = 12.0
    # Rectangle for red side ot1
    rs_coord_x = 0
    rs_corrd_y= 0
    rs_width = 6
    rs_height = 12
    # Rectangle for blue side ot 1
    bs_coord_x = 6
    bs_coord_y = 0
    bs_width = 6
    bs_height = 12
    # Rectangle for water area ot2
    wa_coord_x = 1.95
    wa_coord_y = 1.95
    wa_width = 8
    wa_height = 8
    # Rectangle for inside red side ot 2
    irs_coord_x = 2.55
    irs_coord_y = 2.55
    irs_width = 3.4
    irs_height = 6.8
    # Rectangle for inside blue side ot 3
    ibs_coord_x = 6
    ibs_coord_y = 2.55
    ibs_width = 3.4
    ibs_height = 6.8
    # Rectangle for middle area ot 4
    ma_coord_x = 4.45
    ma_coord_y = 4.45
    ma_width = 3
    ma_height = 3
    # Rectagle small for red side ot 5
    srs_coord_x = 0
    srs_coord_y = 5.25
    srs_width = 1
    srs_height = 1.5
    # Rectangle small for blue side ot 5
    sbs_coord_x = 11
    sbs_coord_y = 5.25
    sbs_width = 1
    sbs_height = 1.5
    # 4 small rectangle white ot 6
    # 1
    sr1_coord_x = 0
    sr1_coord_y = 0
    sr1_width = 0.5
    ...
    ses2_width = 0.5
    ses2_height = 0.5
    # 2 small rectangle moat area ot 8
    #1
    ms1_coord_x = 5.025
    ms1_coord_y = 1.95
    ms1_width = 0.975
    ...
    ms2_width = 0.975
    ms2_height = 0.6
    # 2 small rectangle in middle area ot 9
    #1
    ins1_coord_x = 3.95
    ins1_coord_y = 5.45
    ins1_width = 0.5
...
def animation_robot(hist_x, hist_y, hist_th, vx, vy, index_x, index_y, time_counts):
    ...
    for t in range(len(time_counts)):
        ...
        plt.axis("equal")
        #plt.grid(True)
        plt.title("Velocity of robot [m/sec]:" + str(round(math.sqrt(vx[t]**2+vy[t]**2), 2)))
        plt.pause(0.0001)
...
t0 = 0
mpciter = 0
init_state = np.array([0.975, 6.0, 0.0]).reshape(1,-1)
...
while ((mpciter  * step_horizon < sim_time)):
        ...
        opt_x_x.append(sol_x[0, 0].full())
        opt_x_y.append(sol_x[1, 0].full())
        opt_x_th.append(sol_x[2, 0].full())
        index_pre_x.append(sol_x[0, 0].full())
        index_pre_y.append(sol_x[1, 0].full())
        t0, current_state, state, control = shift_timestep(step_horizon, t0, current_state, sol_x, sol_u, f)
        next_trajectories, next_controls = reference_state_and_control(t0, step_horizon, current_state, N, type="traj1")
        # next_trajectories = np.tile([np.array([2, 2, 0])], N+1)
        # next_controls = np.tile([np.array([10, 10, 10, 10])], N)
        v_x, v_y, v_th = forward_kinematic(sol_u[0, 0].full(), sol_u[1, 0].full(), sol_u[2, 0].full(), sol_u[3, 0].full(), sol_x[2, 0].full())
        vel_x.append(v_x)
        vel_y.append(v_y)
        vel_th.append(v_th)
        vel_m1.append(sol_u[0, 0].full())
        vel_m2.append(sol_u[1, 0].full())
        vel_m3.append(sol_u[2, 0].full())
        vel_m4.append(sol_u[3, 0].full())
        #print(f"x = {np.round(sol_x.full()[0, 0], 3)}, y = {np.round(sol_x.full()[1, 0], 3)}, theta = {np.round(sol_x.full()[2, 0], 3)}")
...
if show_animation == True:
    animation_robot(opt_x_x, opt_x_y, opt_x_th, vel_x, vel_y, index_pre_x, index_pre_y, t)
